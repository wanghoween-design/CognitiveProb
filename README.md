...
|-------|----------|----------|----------|--------------|--------------|--------------|-----------|
| Forward | 2.43 | 1.29 | ↓47.0% | 49.8% | 65.2% | ↑15.4% | 5,898,240 (0.15%) |
| Critical | 2.44 | 1.27 | ↓47.9% | 50.7% | 65.3% | ↑14.6% | 19,464,192 (0.49%) |
| Creative | 2.62 | 1.42 | ↓45.6% | 47.6% | 62.6% | ↑15.0% | 33,030,144 (0.83%) |
...
**核心发现：三个 Agent 的训练效果相近**，Loss 降幅均在 45-48% 之间，Accuracy 升幅均在 14-15% 之间。这说明在当前数据量（324-356 条）和训练轮数（4 epochs）下，参数量的差异没有带来显著的性能差异。

### 2. 训练效率分析

| Agent | 最终 Loss | 最终 Accuracy | 训练轮数 | 训练效率评价 |
|-------|----------|--------------|---------|-------------|
| Forward | 1.29 | 65.2% | 4 epochs | 效果良好，参数最少但表现不错 |
| Critical | 1.27 | 65.3% | 4 epochs | 效果良好，参数适中 |
| Creative | 1.42 | 62.6% | 4 epochs | 效果略差，可能训练不足 |

**三个 Agent 的最终效果相近**，Forward 和 Critical 几乎一样，Creative 略差可能是因为 4 epochs 不够（最优在 Epoch 4.73）。

### 3. 参数量与性能关系

| Agent | 可训练参数 | 占模型比例 | Loss 降幅 | Accuracy 升幅 | 参数效率 |
|-------|-----------|-----------|----------|--------------|---------|
| Forward | 5.9M | 0.15% | 47.0% | 15.4% | 7.9%/M |
| Critical | 19.5M | 0.49% | 47.9% | 14.6% | 2.5%/M |
| Creative | 33.0M | 0.83% | 45.6% | 15.0% | 1.4%/M |

**参数效率递减**：Forward 每百万参数带来 7.9% 的 Loss 降幅，而 Creative 只有 1.4%/M。这符合机器学习的**边际收益递减规律**——参数越多，每增加一单位参数带来的收益越小。
...
|-------|---------------|--------|------|
| Forward | 0.26 ~ 0.71 | ✅ 稳定 | 最稳定，参数最少 |
| Critical | 0.57 ~ 1.09 | ✅ 稳定 | 稳定，有轻微波动 |
| Creative | 0.43 ~ 1.56 | ⚠️ 有波动 | 波动较大，但仍可接受 |
...
*图 1：Forward Agent LoRA 训练过程中的四项核心指标变化。左上图为训练损失（Training Loss），从 2.43 下降至 1.29，降幅 47.0%，表明模型在学习前瞻推理模式；右上图为梯度范数（Gradient Norm），稳定在 0.26-0.71 范围内，无梯度爆炸现象；左下图为策略熵（Policy Entropy），先升后降，反映模型从不确定到确定的学习过程；右下图为平均 Token 准确率（Mean Token Accuracy），从 49.8% 提升至 65.2%，提升 15.4 个百分点。整体训练过程平稳。*

#### 图 2：Critical LoRA 训练指标

![Critical LoRA Training Metrics](adapters/critical_lora/training_metrics_paper_style.png)

*图 2：Critical Agent LoRA 训练过程中的四项核心指标变化。左上图训练损失从 2.44 下降至 1.27，降幅 47.9%；右上图梯度范数在 0.57-1.09 范围内波动，训练稳定；左下图策略熵先升后降，显示模型经历了充分的探索阶段；右下图准确率从 50.7% 提升至 65.3%，提升 14.6 个百分点。Critical 注入了 6 个目标模块（含 gate_proj 和 up_proj），可训练参数达 19.5M。*

#### 图 3：Creative LoRA 训练指标

![Creative LoRA Training Metrics](adapters/creative_lora/training_metrics_paper_style.png)

*图 3：Creative Agent LoRA 训练过程中的四项核心指标变化。左上图训练损失从 2.62 下降至 1.42，降幅 45.6%；右上图梯度范数在 0.43-1.56 范围内波动，波动较大但仍在健康范围；左下图策略熵先升后降，探索充分；右下图准确率从 47.6% 提升至 62.6%，提升 15.0 个百分点。Creative 注入了全部 7 个目标模块（含 down_proj），可训练参数达 33.0M，是 Forward 的 5.6 倍。*

#### 图 4：三个 Agent 训练结果横向对比

![LoRA Training Comparison](adapters/training_comparison.png)

*图 4：三个 LoRA Agent 训练结果的横向对比。橙色线为 Forward（r=8），蓝色线为 Critical（r=12），绿色线为 Creative（r=16）。左上图显示训练损失对比，三者的最终损失相近（Forward 1.29, Critical 1.27, Creative 1.42）；右上图显示准确率对比，三者的最终准确率也相近（Forward 65.2%, Critical 65.3%, Creative 62.6%）；左下图显示梯度范数对比，Creative 的波动最大；右下图显示策略熵对比。该图展示了在当前训练条件下，三个 Agent 的性能差异不大。*
...
4. **增加训练轮数**：Creative 的最优在 Epoch 4.73，可以尝试 5 epochs
